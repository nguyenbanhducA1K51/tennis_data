...
const DropDown = ({ searchTour }) => {
  ...
  const tour = useRef({ name: "Wimbledon", year: 2017 });
  const tournamentList = [
    ...
    "Tournament of Champions WCT",
    "Columbus WCT",
    "Delray Beach Open",
    "Khartoum",
    "Sydney",
    "ATP Doubles Challenge Cup",
    "Metz",
    "Rome WCT",
    "Munich WCT",
  ...
  const [input, setInput] = useState({ tourname: "", touryear: "2017" });
  const yearList = [
  ...
  return (
    <div>
      ...
      <div className="d-flex justify-content-center mt-3">
        <button className=" btn btn-success " onClick={findTour}>
          {" "}
          submit
        </button>
      </div>
    ...
    <>
      <div className="d-flex mt-3">
        ...
        <div style={{ marginLeft: "100px" }}>
          ...
          >
            {tourNameInput.length == 0 || allTour.includes(tourNameInput) ? (
              <li></li>
            ) : (
              tourList.map((tour) => (
                <li className="bg-white" key={tour} onClick={() => click(tour)}>
                  {tour}
                </li>
              ))
            )}
...
const InputTourYear = ({
  ...
  return (
    <>
      <div className="d-flex mt-3">
        <label for="number" className="d-flex justify-content-center mr-2  ">
          {" "}
          search for year from 1877 to 2017
        </label>
        <div>
          {/* onfocusout={()=>{}} */}
          <input
